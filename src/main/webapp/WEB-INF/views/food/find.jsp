    ...
    <section class="product-details spad" id="food_find">
        <div class="container">
        	<div class='row' style="margin:0px auto">
        		<input type=text size=20 class="input-sm" placeholder="주소를 입력하세요" v-model="store.address">
            ...
            <div class="row" style="margin-top: 10px">
              <div class="product__pagination" style="margin: 0px auto">
                  <a v-if="store.startPage>1" @click="store.pageChange(store.startPage-1)" style="cursor:pointer">&laquo;</a>
                  <a v-for="i in store.range(store.startPage,store.endPage)" @click="store.pageChange(i)" style="cursor:pointer">{{i }}</a>
                  <a v-if="store.endPage<store.totalPage" @click="store.pageChange(store.endPage+1)" style="cursor:pointer">&raquo;</a>
              </div>
            </div>
        </div>
    </section>
